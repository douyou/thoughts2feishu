...
const defaultBaseURL = "https://thoughts.aliyun.com"
...
func NewRequest(cookie string, firstHash string) *request {
	return NewRequestWithBase(cookie, firstHash, defaultBaseURL)
}

func NewRequestWithBase(cookie string, firstHash string, baseURL string) *request {
	if baseURL == "" {
...
func (r *request) GetWorkspace(hash string) (*Workspaces, error) {
	...
	url := fmt.Sprintf("%s/api/workspaces/%s?pageSize=1000&_=%d", r.baseURL, hash, utils.UnixTimstampMillisecond())
	// fmt.Println(url)
...
func (r *request) EnableOutput(hash string, enable bool) (bool, error) {
	...
	url := fmt.Sprintf("%s/api/workspaces/%s/workspaceSecurity", r.baseURL, hash)
...
func (r *request) GetNodesByHash(hash string, prefixPath string) ([]*Node, error) {
	...
	url := fmt.Sprintf("%s/api/workspaces/%s/nodes?pageSize=1000%s&_=%d", r.baseURL, r.firstHash, parentHash, utils.UnixTimstampMillisecond())
	// fmt.Println(url)
	content, _, err := req.Get(url)
	...
	for _, node := range result.Nodes {
		// Title 中的 / 不能当路径分隔符，否则会拆成错误目录（如 SPU/SKU）
		node.Path = fmt.Sprintf("%s/%s", prefixPath, sanitizePathSegment(node.Title))
	}

	return result.Nodes, nil
}

// sanitizePathSegment 将文档标题中的路径分隔符替换为全角字符，避免落盘时被拆成多级目录。
...
func (r *request) GetDownloadUrl(hash string, prefixPath string, fileType string) (*NodeDownload, error) {
	...
	url := fmt.Sprintf("%s/convert/api/nodes/%s/export:%s?pageSize=1000&_=%d", r.baseURL, hash, fileType, utils.UnixTimstampMillisecond())
	...
	url = fmt.Sprintf("%s/convert/api/exportDocx:polling?pageSize=1000&id=%s&_=%d", r.baseURL, result.ID, utils.UnixTimstampMillisecond())
...
func (r *request) GetDownloadUrlByDetail(hash string, prefixPath string) (*NodeDownload, error) {
	...
	url := fmt.Sprintf("%s/api/workspaces/%s/nodes/%s?pageSize=1000&_=%d", r.baseURL, r.firstHash, hash, utils.UnixTimstampMillisecond())
