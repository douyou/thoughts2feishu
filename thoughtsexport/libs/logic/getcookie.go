...
// WaitLoginReturnCookieString 循环监听指定 cookie key 的状态（支持逗号分隔多个候选 key）
func WaitLoginReturnCookieString(ctx context.Context, waitCookieKey string) string {
	keys := strings.Split(waitCookieKey, ",")
	for i := range keys {
		keys[i] = strings.TrimSpace(keys[i])
	}

	log.Printf("等待登录 Cookie，候选 key: %v（请在弹出的 Chrome 窗口完成登录）", keys)
	...
	for {
		...
		for _, cookie := range cookies {
			...
			for _, key := range keys {
				if key != "" && cookie.Name == key {
					waitCookieKeyExist = true
					log.Printf("已检测到登录 Cookie: %s", key)
				}
			}
		}
		...
		tick++
		// 每 30 秒打印一次当前 cookie 名，便于排查云效实际登录态字段
		if tick%30 == 0 {
			log.Printf("仍未匹配到登录 Cookie。当前浏览器 Cookie 名: %v", names)
		}
